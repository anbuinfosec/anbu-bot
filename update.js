...
const baseURL = "https://raw.githubusercontent.com/anbuinfosec/anbu-bot/main";
const allVersionsURL = "https://raw.githubusercontent.com/anbuinfosec/AnbuBotControls/main/versions.json";
    ...
    try {
        ...
        if (newVersionIndex != -1) {
            for (const [, versionAfter] of Object.entries(data).slice(newVersionIndex)) {
                const scriptsURL = `https://raw.githubusercontent.com/anbuinfosec/AnbuBotControls/main/${versionAfter}.json`;
        ...
        logger.error('[ ANBUBOT ] Failed to check for updates.');
        logger.error(err);
        process.exit(1);
    }
}
...
const update = (scripts, newPackage) => {
    try {
        const { changed, added, removed } = scripts;
        logger.custom("[i] Updating...", "UPDATE");
        const backup = resolve("./backup");
...
const main = async () => {
    try {
        ...
        if (updateScriptsArr.length == 0) {
            ...
            console.log(text);
            logger.warn("[ ANBUBOT ] Please check the above changes and backup if necessary.");
            logger.warn("[ ANBUBOT ] Folder 'backup' will be used to backup old files.");
            ...
            rl.question("» Do you want to update? (y/n) ", answer => {
                if (answer.toLowerCase() === "y") {
                    ...
                    rl.close();
                    logger.warn("[ ANBUBOT ] will not be updated.");
                }
            })
        }
    } catch (e) {
        logger.error(e);
        logger.error("[ ANBUBOT ] An error occured, try again later..");
        process.exit(0);
    }
};
